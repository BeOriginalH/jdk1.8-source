...
public class TreeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, java.io.Serializable{
    ...
    private void fixAfterInsertion(Entry<K, V> x) {
        ...
        //当插入节点的父节点是红色的时候，需要进行自平衡操作
        while (x != null && x != root && x.parent.color == RED) {
            // case 1 : 父节点是祖父节点的左子树节点
            if (parentOf(x) == leftOf(parentOf(parentOf(x)))) {
                //获取叔叔节点
                Entry<K, V> y = rightOf(parentOf(parentOf(x)));
                // case 1.1 : 叔叔节点是红色，设置父节点为黑色，叔叔节点为黑色，祖父节点为红色，将祖父节点设置为插入的节点
                if (colorOf(y) == RED) {
                    ...
                    // case 1.2 :叔叔节点是黑色或者不存在
                } else {
                    // case 1.2.1 : 插入节点是父节点的右子节点，对父节点进行左旋操作，并把父节点作为插入节点进行处理，进行1.2.2场景处理
                    if (x == rightOf(parentOf(x))) {
                        x = parentOf(x);
                        rotateLeft(x);
                    }
                    // case 1.2.2 : 插入节点是父节点的左子节点，此时将父节点设置为黑色，祖父节点设置为红色，进行右旋操作
                    setColor(parentOf(x), BLACK);
                    setColor(parentOf(parentOf(x)), RED);
                    rotateRight(parentOf(parentOf(x)));
                }
            } else {
                //case 2 : 父节点是祖父节点的右子节点

                //获取叔叔节点
                Entry<K, V> y = leftOf(parentOf(parentOf(x)));

                // case 2.1 : 叔叔节点是红色，设置父节点和叔叔节点为黑色，祖父节点为红色，把祖父节点设置为当前处理节点，递归处理
                if (colorOf(y) == RED) {
                    setColor(parentOf(x), BLACK);
                    setColor(y, BLACK);
                    setColor(parentOf(parentOf(x)), RED);
                    ...
                    // case 2.2  叔叔节点是黑色或者不存在
                } else {
                    // case 2.2.1 : 插入节点是父节点的左子节点，对父节点进行右旋旋操作，并把父节点作为插入节点进行处理，进行2.2.2场景处理
                    if (x == leftOf(parentOf(x))) {
                        x = parentOf(x);
                        rotateRight(x);
                    }
                    // case 2.2.2 : 插入节点是父节点的右子节点，此时将父节点设置为黑色，祖父节点设置为红色，进行左旋操作
                    setColor(parentOf(x), BLACK);
                    setColor(parentOf(parentOf(x)), RED);
